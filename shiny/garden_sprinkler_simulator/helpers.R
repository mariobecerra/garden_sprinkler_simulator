...
create_random_design = function(n_runs = 10, file_out_name = NULL, seed = NULL, truncate_digits = NULL){
  ...
  for(j in 1:nrow(limits)){
    if(!is.null(truncate_digits)){
      out_df[, j] = round(runif(n_runs, limits$min[j], limits$max[j]), truncate_digits)
    } else{
...
sprinkler = function(
  design_matrix = NULL, # data.frame with a design
  ...
  # is design_matrix argument is not null
